...
func (tx *Tx) init() {
	tx.CommentContent = NewCommentContentClient(tx.config)
	tx.CommentIndex = NewCommentIndexClient(tx.config)
	tx.CommentSubject = NewCommentSubjectClient(tx.config)
}
...
// applies a query, for example: CommentContent.QueryXXX(), the query will be executed
